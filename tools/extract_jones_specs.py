# -*- coding: utf-8 -*-
"""从 Jones 2026-27 中文目录 PDF (p134-138) 提取雪板规格表 -> data/jones-specs.json

列布局来自 PDF 表头（竖排文字）与数据词的 x 坐标锚点。
分类映射来自 p30/p31 视觉总览页与 p133 Big Horn 速查表。
"""
import json
import re
...
OUT = Path(__file__).resolve().parent.parent / "data" / "jones-specs.json"
...
# 型号 -> 滑行分类（源自 p30/p31 视觉总览、p133 Big Horn 速查表）
CATEGORY = {
...
def parse_range(s):
    """'54-77'/'67-91+'/'10+'/'42-44.5' -> (min, max, open_ended) 或 None"""
...
def main():
    ...
    models = []
    current = None
    for pno in range(133, 138):  # PDF 第 134-138 页
        rows = {}
    ...
    for m in merged:
        ...
        for s in m["sizes"]:
            ...
            row = {
                ...
                "variant": size_m.group(2) or "",  # W=宽 N=窄 UW=超宽
    ...
    for m in out:
        tag = " [NEW]" if m["is_new"] else ""
        print(f"  {m['audience']}·{m['category']}{tag} {m['name']}: "
              f"{', '.join(s['size'] for s in m['sizes'])}")
